...
function createElevationSelector(){
    ...
    inner_text += "<td><p class='defShadow'>" +getZaxisText();
    inner_text += "&nbsp;<button type='button' class='closeButton'  onclick='displayElevationSelector()'>x</button></p></td></tr>";
    ...
    for(var page = 1; page <=totPages; page++){
        ...
        {	
            ...
            {
                inner_text += "<input onclick='changeElev(" + i + ")' type='radio' name='elev_select' value='" 
...
function displayElevationSelector() {
    ...
    if( zAxis_span_visible ){
        area.style.visibility = "hidden";
        zAxis_span_visible = false;
    }else{
        area.style.visibility = "visible";
        zAxis_span_visible = true;
    }   
...
function changeElev(value)
{
    ...
    getMainLayer().mergeNewParams({
        elevation:layerDetails.zaxis.values[elev_glob_counter]
    });
