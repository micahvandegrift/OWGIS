...
function loadPalettes(){
	...
	if(mapConfig['menuDesign'] == "topMenu"){
		...
		document.getElementById('palette').style.left = (widthNum - 250) + "px"
	}
        if(mapConfig['menuDesign'] == "sideMenu")
        {
            document.getElementById('palettes-div').style.left = 180 + "px"
		document.getElementById('palette').style.left= 30 + "px"
        }
}
 ...
 */
function displayOptionalPalettes(force_visib){
	...
	if( anim_loaded && !stoppedAnimation ){
		...
		//If the animation is not running we toggle the visibility of the optional palettes
		if(force_visib == undefined){
			palet_vis = $('#palettes-div').css("visibility");
			if(palet_vis == "visible"){
				$('#palettes-div').css("visibility","hidden");
			}
			else{
				$('#palettes-div').css("visibility","visible");
			}
		}else{
			$('#palettes-div').css("visibility",force_visib);
		}
	}
}
...
{       
	...
	$('#palette').css("display",pal_visib);    
	displayOptionalPalettes(force);
}
