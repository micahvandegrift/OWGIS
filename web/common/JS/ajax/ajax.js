		...
		case "dispAnimation":
			...
			if (previous_url == url3 && finishedLoading == true)
			{
				...
				if (stoppedAnimation == true && anim_loaded == false)
				{
					...
					animation_layer.setVisibility(true);
...
function AsyncPunctualData(responseText) {
	...
	var isLink = -1;//We are doing anything with the links now, only displaying them
	if (isLink != -1) {//If the result is a link, we open the corresponding page
		newLink = responseText.substr(isLink, responseText.length);//text with link from begining to end
		endOfLink = minimo(newLink.indexOf(' '), newLink.indexOf('<'));//index where link finishes
		newLink = newLink.substr(0, endOfLink);//this is only link

		window.open(newLink, "blank");
	} else {//This is the default case.
		if (myWCSpopup.contentHTML == "Loading ...") {// TODO fix this is hard coded very bad
			finalText = responseText;
		} else {
			finalText = myWCSpopup.contentHTML + responseText;
		}
		myWCSpopup.setContentHTML(finalText);

		myWCSpopup.updateSize();
		myWCSpopup.panIntoView();
		popSize = document.getElementById(myWCSpopup.div.id).style.height;//get id of popup box


		var fraction = parseInt(popSize) * 1.00;
		document.getElementById(myWCSpopup.div.id).style.height = fraction.toString() + 'px';
	}
...
function asyncFillAnimationSelect(responseText) {
	...
	var IE_options = ''; //the .append() function doesnt work in IE so it is using a string to create the options

	if (parseInt(IE_version) <= 8)
	{
		isIE8orLess = true;
	...
	for (var key in animOpts.timeStrings) {
		...
		if (isIE8orLess)
		{
			IE_options += "<option value=" + timeStr + ">" + title + "</option>";
		}
	...
	//fix of above bug. .append()
	if (isIE8orLess)
	{
		$('#timeSelect').html(IE_options);
	}
